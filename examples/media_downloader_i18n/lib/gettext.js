...
bt.Gettext = {
	...
	initialize: function(lang) {
		...
		$('link').each(function(idx, link){
			if (link.rel == 'gettext' && link.href && link.lang){
				bt.Gettext.links[idx] = [link.lang, link.href];
				bt.Gettext.include(link.lang, link.href);
			}
		});
	},
	...
	gettext: function() {
		...
		if (bt.Gettext.LCmessages[bt.Gettext.lang] && $.inArray(str, bt.Gettext.LCmessages[bt.Gettext.lang].msgid) != -1) {
				str = bt.Gettext.LCmessages[bt.Gettext.lang].msgstr[$.inArray(str, bt.Gettext.LCmessages[bt.Gettext.lang].msgid)];
		}
	...
	parse: function(str) {
		...
		function clean(str) {
			//console.log(str);
			var newstr =  str.replace(rgx_msg,'').replace("msgid ", "").replace("msgstr ", "").replace(/\"/g,'');
			//console.log(newstr);
			return newstr;
		}
